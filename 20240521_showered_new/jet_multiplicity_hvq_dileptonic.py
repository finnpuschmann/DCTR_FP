...
P0 = np.array(P0)
P1 = np.array(P1)
np.save(f'./output/ShowerTop_hvq_dileptonic.npy', P0)
print(f'{np.shape(P0) = }')

np.save(f'./output/ShowerTT_hvq_dileptonic.npy', P1)
print(f'{np.shape(P1) = }')


# save multiplicity and jet observables
np.save(f'./output/jet_multiplicity_hvq_dileptonic.npy', nJets)
print(f'{np.shape(nJets) = }')

np.save(f'./output/jet_4vectors_hvq_dileptonic.npy', jets_4vectors)
