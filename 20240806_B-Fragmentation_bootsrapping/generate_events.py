...
# argv gives the commandline arguments when calling
# python file.py bla bli blub
# argv[0] is first argument; file.py
# argv[1] is second; 	     bla    , etc.
try:
    Rb = float(sys.argv[1])
except:
    print('No Argument for Rb passed, using default value of 0.855')
    Rb = 0.855

try:
    seed = int(sys.argv[2])
except:
    print('No Argument for Seed passed, using random seed between [1, 1000]')
    seed = randint(1, 1000)
    
try:
    N = int(sys.argv[3])
except:
    print('No Argument for Number of events passed, using default 1 Million')
    N = int(1e6)
...
np.savez(f'{dir}/bootstrapping_pT:bfirst_blast_abfirst_ablast_Bpos_Bneg_top_antitop-Rb_{Rb}_{num}_seed{seed}_CP5.npz', a=P5, b=P6, c=P7, d=P8, e=P9, f=P10, g=P11, h=P12)
np.savez(f'{dir}/bootstrapping_AllP4:TopP-antiTopP-bFirstP-bLastP-abFirstP-abLastP-BposP-BnegP-Rb_{Rb}_{num}_seed{seed}_CP5.npz', a=Pall)
np.savez(f'{dir}/bootstrapping_AllP4:TopP-antiTopP-bLastP-abLastP-BposP-BnegP-wplus-wminus-Rb_{Rb}_{num}_seed{seed}_CP5.npz', a=Pall)
