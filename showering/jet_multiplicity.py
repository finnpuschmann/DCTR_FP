...
R = 0.4  # Jet radius
jet_def = pythia8.SlowJet(-1, R, 10., 5.)
...
for iEvent in range(0, N):
    ...
    # jet multiplicity
    # Create a list to store final state particles for jet clustering
    particles = []
    
    # Fill particles list with final state particles
    for prt in pythia.event:
        if prt.isFinal() and prt.isCharged():
            particles.append(fj.PseudoJet(prt.px(), prt.py(), prt.pz(), prt.e()))

    # Perform jet clustering
    jets = fj.sorted_by_pt(jet_def(particles))

    # Calculate jet multiplicity and store it
    jet_multiplicity = len(jets)
    jet_multiplicities.append(jet_multiplicity)
...
np.save('jet_multiplicities.npz', jet_multiplicities)
print(np.shape(jet_multiplicities))
print(jet_multiplicities[:20])
